...
watch}= useForm<OnboardingFormData>({
...
const onSubmit= async(values: OnboardingFormData)=>{
 const skills = values.skills
  ...
  const finalValues = {
    ...values,
    skills, // Now it's a string[]
  };

  try {
    const formattedIndustry= `${finalValues.industry}-${finalValues.subIndustry.toLowerCase().replace(/ /g,"-")}`;
    await updateUserFn({
      ...finalValues,
...
useEffect(()=>{
 if (updateResult?.updateUser && !updateLoading){
        ...
        <Label htmlFor="industry"> Industry</Label>
         <Select
            onValueChange={(value) => {
                  ...
                  const foundIndustry = industries.find((ind) => ind.id === value);
  if (foundIndustry) {
    ...
    setSelectedIndustry(null);
         ...
         >
          <SelectTrigger id="industry" className="mb-2">
            <SelectValue placeholder="Select an industry" className="mt-2"></SelectValue>
          </SelectTrigger>
          <SelectContent>
            {industries.map((ind)=>{
        ...
        <Textarea id="bio" placeholder="Tell us about your professional background.." className="h-32"
        {...register("bio")}
        />
